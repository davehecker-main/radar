...
dependencies on other sessions instead. Active claims are the ownership record, complete
whatever the announcement history holds; a claim whose owner is idle, unknown, or stale is
still held, and one absent from a later list is released. A free-text announcement occupies
work only when its sender is present in the current, coverage-complete `chattr who --repo`
list; an absent or `gone` sender reserves nothing. A full or rolled-over `broadcasts` window
is not an ownership gap.

With complete coverage and a `claims` array, withhold only the unresolved resource's row
from Unattended (`issue:<N>`, `pr:<N>`, or `resource:<name>`); put its uncertainty in
Primary blockers. Only `coverage.complete: false` or a missing `claims` array withholds
the whole Unattended list; name which condition failed in Primary blockers. Do not prescribe
polling, a peer survey, or asking Dave to reconstruct ownership.
